...
class Stylesheet(object):
    ...
    #      
    #    class Stylet {
    #      Array<CSSProperty> properties;
    #    
    #      Stylet(this.properties) { }
    #    }
    #
    # TODO(terry): Stylet should have the class name too. Should be able to go
    # from the class name to the stylet. Ideally only pass one parameter to
    # the super constructor for View.
    #      
    #    class Layout1 extends Stylet {
    #      mycss() : super([new CSSProperty['a', 'v1'], ['b', 'v2']]) {}
    #    }

    # TODO(terry): Place classes in framework.
    #
    ...
    #
    #  class CssLayout extends Stylet {
    #    CssLayout() : super([const CSSProperty('propName', 'propValue')]) {}
    #  }
    def emitStyletClass(self, stylet):
      self.styletClasses.append('class STYLET_{0} extends CSSStylet {{\n'.
                                format(stylet.name))
      self.styletClasses.append('  STYLET_{0}() : super([\n'.
                                format(stylet.name))
      return stylet.name

    # Emit each property in the stylet properties array:
    #    const CSSProperty('propName', 'propValue')
    def emitStyletProperty(self, decl, anyProps):
      ...
      if propValues > 0:
        ...
        for value in decl[-(propValues - 2):]:
          ...
          first = False
        self.styletClasses.append("      const CSSProperty('{0}', '{1}')".
    ...
    def dartClass(self, sourceFile, cssName, cssFiles):
        ...
        for node in nodes:
          if isinstance(node, Ruleset):
            ...
            for selectorTree in ruleset.data:
              ...
              elif (isinstance(stylet, Stylet) and
                    isinstance(selectorTree, Declaration)):
                anyProps = self.emitStyletProperty(selectorTree.data, anyProps)

            # Done with this selector if stylet close up this stylet class.
            if isinstance(stylet, Stylet):
      ...
      # Emit all stylet instances in this CSS file.
      self.cssClass.append('\n  // CSS stylet instances:\n')
      for styletName in stylets:
        self.cssClass.append(self.emitStyletInstance(styletName));

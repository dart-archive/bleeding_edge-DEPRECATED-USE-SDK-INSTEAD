/**
 * @fileoverview Codemirror 2 mode for Dart.
 * @author pquitslund@google.com (Phil Quitslund)
 * @author brianwilkerson@google.com (Brian Wilkerson)
 * @license
 *
...
CodeMirror.defineMode("dart", function(config, parserConfig) {
  ...
  function dartTokenBase(stream, state) {
    var ch = stream.next();
    if (ch == '"' || ch == "'") {
      state.quote = getQuote(stream, ch);
      state.raw = false;
      ...
      ch = stream.peek();
      if (ch == '"' || ch == "'") {
        state.quote = getQuote(stream, stream.next());
        state.raw = true;
        return chain(stream, state, dartTokenString);
      }

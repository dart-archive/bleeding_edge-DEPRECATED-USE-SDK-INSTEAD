...
//        }
  ...
  private void ignoreBuildDir(IProgressMonitor monitor) {
    DartIgnoreManager ignoreManager = DartCore.getProjectManager().getIgnoreManager();
    IFolder buildDir = container.getFolder(new Path("build"));
    try {
      ignoreManager.addToIgnores(buildDir.getLocation());
      ((IResource) buildDir).setDerived(true, monitor);
    } catch (IOException e) {
      DartCore.logError("Failed to ignore: " + buildDir, e);
    } catch (CoreException e) {

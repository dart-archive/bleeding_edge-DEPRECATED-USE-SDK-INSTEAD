...
public final class IndentManipulation {
   ...
   * <p>
   * The first line of the code will not be changed (It is considered to have no indent as it might
   ...
   *              </ul>
   */
  public static String changeIndent(String code, int indentUnitsToRemove, int tabWidth,
      int indentWidth, String newIndentString, String lineDelim) {
    if (tabWidth < 0 || indentWidth < 0 || code == null || indentUnitsToRemove < 0
        || newIndentString == null || lineDelim == null) {
    ...
    try {
      ...
      for (int i = 0; i < nLines; i++) {
        ...
        if (i == 0) { // no indent for first line (contained in the formatted
                      // string)
          buf.append(line);
  ...
  /**
   * Returns the leading indentation string of the given line. Note that the returned string need
   * not be equal to the leading whitespace as odd spaces are not considered part of the
   * indentation.
   * 
  ...
  /**
   * Returns the text edits retrieved after changing the indentation of a, possible multi-line, code
   * string.
   * <p>
   ...
   *              </ul>
   */
  public static ReplaceEdit[] getChangeIndentEdits(String source, int indentUnitsToRemove,
   ...
   * <p>
   * Use {@link DartProject#getOptions(boolean)} to get the most current project options.
   * </p>
   * 
  ...
  /**
   * Removes the given number of indentation units from a given line. If the line has less indent
   * than the given indentUnitsToRemove, all the available indentation is removed. If
   * <code>indentsToRemove <= 0 or indent == 0</code> the line is returned.
   * 
   ...
   *              </ul>
   */
  public static String trimIndent(String line, int indentUnitsToRemove, int tabWidth,
      int indentWidth) {
    if (tabWidth < 0 || indentWidth < 0 || line == null) {
      throw new IllegalArgumentException();
    }
   ...
   */
  private static int indexOfIndent(CharSequence line, int numberOfIndentUnits, int tabWidth,
      int indentWidth) {

...
//
//    private static List<TypeNameMatch> findTypeInfos(String typeName, IType contextType,
//        IProgressMonitor pm) throws JavaModelException {
//      IJavaSearchScope scope = SearchEngine.createJavaSearchScope(
//          new IJavaProject[] {contextType.getJavaProject()},
//          true);
...
//    }
//
//    private static String resolveType(String elementTypeName, RefactoringStatus status,
//        IType declaringType, IProgressMonitor pm) throws CoreException {
...
//
//    public MethodTypesChecker(IMethod method, StubTypeContext stubTypeContext,
...
//      RefactoringStatus[] results = new MethodTypesSyntaxChecker(
//          fMethod,
//          fParameterInfos,
//          fReturnTypeInfo).checkSyntax();
...
//    }
//
//    private ITypeBinding[] resolveBindings(String[] types, RefactoringStatus[] results,
//        boolean firstPass) throws CoreException {
...
//        CompilationUnit compilationUnit = new RefactoringASTParser(ASTProvider.SHARED_AST_LEVEL).parse(
//            wc,
//            true);
//        ASTNode method = NodeFinder.perform(
//            compilationUnit,
//            offsetBeforeMethodName,
//            METHOD_NAME.length()).getParent();
...
//    private final ReturnTypeInfo fReturnTypeInfo;
//
//    public MethodTypesSyntaxChecker(IMethod method, List<ParameterInfo> parameterInfos,
...
//        return null;
//      }
//      return TypeContextChecker.checkParameterTypeSyntax(
//          info.getNewTypeName(),
...
//        String msg = Messages.format(
//            RefactoringCoreMessages.TypeContextChecker_invalid_return_type_syntax,
//            keys);
//        result.addError(msg);
//      }
//      return result;
...
//  }
//
//  public static RefactoringStatus[] checkAndResolveMethodTypes(IMethod method,
//      StubTypeContext stubTypeContext, List<ParameterInfo> parameterInfos,
//      ReturnTypeInfo returnTypeInfo) throws CoreException {
//    MethodTypesChecker checker = new MethodTypesChecker(
//        method,
//        stubTypeContext,
//        parameterInfos,
//        returnTypeInfo);
...
//  }
//
//  public static RefactoringStatus[] checkMethodTypesSyntax(IMethod method,
//      List<ParameterInfo> parameterInfos, ReturnTypeInfo returnTypeInfo) {
//    MethodTypesSyntaxChecker checker = new MethodTypesSyntaxChecker(
//        method,
//        parameterInfos,
//        returnTypeInfo);
...
//      return RefactoringStatus.createFatalErrorStatus(msg);
    ...
    boolean valid = false;
    try {
      Type type = unit.getLibrary().findTypeInScope(newTypeName);
      valid = type != null;
    } catch (DartModelException e) {
    }

    if (!valid) {
      String typeLabel = BasicElementLabels.getDartElementName(newTypeName);
      String msg = Messages.format(
          RefactoringCoreMessages.TypeContextChecker_invalid_type_name,
          typeLabel);
      return RefactoringStatus.createFatalErrorStatus(msg);
    }
...
//      String msg = Messages.format(
//          RefactoringCoreMessages.TypeContextChecker_no_vararg_below_50,
//          typeLabel);
//      return RefactoringStatus.createFatalErrorStatus(msg);
//    }
//
...
//    for (Iterator<String> iter = problemsCollector.iterator(); iter.hasNext();) {
//      String msg = Messages.format(
//          RefactoringCoreMessages.TypeContextChecker_invalid_type_syntax,
//          new String[] {
//              BasicElementLabels.getJavaElementName(newTypeName),
//              BasicElementLabels.getJavaElementName(iter.next())});
//      result.addError(msg);
...
//  }
//
//  public static StubTypeContext createStubTypeContext(ICompilationUnit cu, CompilationUnit root,
//      int focalPosition) throws CoreException {
...
//  }
//
//  public static StubTypeContext createSuperClassStubTypeContext(String typeName,
//      IType enclosingType, IPackageFragment packageFragment) {
//    return createSupertypeStubTypeContext(typeName, false, enclosingType, packageFragment);
//  }
//
//  public static StubTypeContext createSuperInterfaceStubTypeContext(String typeName,
//      IType enclosingType, IPackageFragment packageFragment) {
...
//  }
//
//  public static ITypeBinding resolveSuperClass(String superclass, IType typeHandle,
//      StubTypeContext superClassContext) {
//    StringBuffer cuString = new StringBuffer();
...
//
//    try {
//      ICompilationUnit wc = typeHandle.getCompilationUnit().getWorkingCopy(new WorkingCopyOwner() {/*subclass*/
//      }, new NullProgressMonitor());
//      try {
//        wc.getBuffer().setContents(cuString.toString());
//        CompilationUnit compilationUnit = new RefactoringASTParser(ASTProvider.SHARED_AST_LEVEL).parse(
//            wc,
//            true);
//        ASTNode type = NodeFinder.perform(
//            compilationUnit,
//            superClassContext.getBeforeString().length(),
//            superclass.length());
...
//  }
//
//  public static ITypeBinding[] resolveSuperInterfaces(String[] interfaces, IType typeHandle,
//      StubTypeContext superInterfaceContext) {
...
//  }
//
//  private static StubTypeContext createSupertypeStubTypeContext(String typeName,
//      boolean isInterface, IType enclosingType, IPackageFragment packageFragment) {
...
//        stubTypeContext = new StubTypeContext(
...
//      stubTypeContext = new StubTypeContext(
//          cu,
//          "package " + packageFragment.getElementName() + ";" + prolog, epilog); //$NON-NLS-1$//$NON-NLS-2$
//
//    } else {
//      stubTypeContext = new StubTypeContext(null, null, null);
//    }
...
//  }
//
//  private static void fillWithTypeStubs(final StringBuffer bufBefore, final StringBuffer bufAfter,
//      final int focalPosition, List<? extends BodyDeclaration> types) {
//    StringBuffer buf;
...
//    }
//    if (selected.getStartPosition() != offset || selected.getLength() != superType.length()
//        || !(selected instanceof Type) || selected instanceof PrimitiveType) {
//      return null;
//    }
...
//  }
//
//  private static Type parseType(String typeString, IJavaProject javaProject,
//      List<String> problemsCollector) {

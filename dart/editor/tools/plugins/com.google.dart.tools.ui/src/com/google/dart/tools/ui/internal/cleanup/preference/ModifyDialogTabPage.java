...
public abstract class ModifyDialogTabPage implements IModifyDialogTabPage {
  ...
  protected class ButtonPreference extends Preference {
     ...
     */
    public ButtonPreference(Composite composite, int numColumns, Map<String, String> preferences,
        String key, String[] values, String text, int style) {
  ...
  protected final class CheckboxPreference extends ButtonPreference {
    public CheckboxPreference(Composite composite, int numColumns, Map<String, String> preferences,
        String key, String[] values, String text) {
  ...
  protected final class ComboPreference extends Preference {
     ...
     */
    public ComboPreference(Composite composite, int numColumns, Map<String, String> preferences,
        String key, String[] values, String text, String[] items) {
      super(preferences, key);
   ...
   * sessions. Each ModifyDialogTabPage has its own instance, and it should add all relevant
   * controls upon creation, always in the same sequence. This established a mapping of controls to
   * indexes, which allows to restore the focus in a later session. The index is saved in the dialog
   * settings, and there is only one common preference for all tab pages. It is always the currently
   * active tab page which stores its focus index.
   */
  protected final static class DefaultFocusManager extends FocusAdapter {

    private final static String PREF_LAST_FOCUS_INDEX = DartUI.ID_PLUGIN
  ...
  protected final class NumberPreference extends Preference {
     ...
     * @param text The label text for this Preference.
     */
    public NumberPreference(Composite composite, int numColumns, Map<String, String> preferences,
        String key, int minValue, int maxValue, String text) {
      super(preferences, key);
  ...
  /**
   * The base class of all Preference classes. A preference class provides a wrapper around one or
   * more SWT widgets and handles the input of values for some key. On each change, the new value is
   * written to the map and the listeners are notified.
   */
  protected abstract class Preference extends Observable {
    ...
    /**
     * Returns the main control of a preference, which is mainly used to manage the focus. This may
     * be <code>null</code> if the preference doesn't have a control which is able to have the
     * focus.
     * 
    ...
    /**
     * To be implemented in subclasses. Update the SWT widgets when the state of this object has
     * changed (enabled, key, ...).
     */
  ...
  protected final class RadioPreference extends ButtonPreference {
    public RadioPreference(Composite composite, int numColumns, Map<String, String> preferences,
        String key, String[] values, String text) {
  ...
  protected final class StringPreference extends Preference {
     ...
     */
    public StringPreference(Composite composite, int numColumns, Map<String, String> preferences,
        String key, String text, IInputValidator inputValidator) {
      super(preferences, key);
    ...
    private IStatus createErrorStatus(String errorText) {
      return new Status(IStatus.ERROR, DartToolsPlugin.getPluginId(), 0, errorText, null);
  ...
  /**
   * Layout used for the settings part. Makes sure to show scrollbars if necessary. The settings
   * part needs to be layouted on resize.
   */
  ...
  /**
   * This is the default listener for any of the Preference classes. It is added by the respective
   * factory methods and updates the page's preview on each change.
   */
  ...
  /**
   * The default focus manager. This widget knows all widgets which can have the focus and listens
   * for focusGained events, on which it stores the index of the current focus holder. When the
   * dialog is restarted, <code>restoreFocus()</code> sets the focus to the last control which had
   * it. The standard Preference object are managed by this focus manager if they are created using
  ...
  /*
   * Convenience method to create a CheckboxPreference. The widget is registered as a potential
   * focus holder, and the default updater is added.
   */
  ...
  /*
   * Convenience method to create a ComboPreference. The widget is registered as a potential focus
  ...
  /*
   * Create a GridLayout with the default margin and spacing settings, as well as the specified
   * number of columns.
   */
  ...
  /*
   * Convenience method to create a NumberPreference. The widget is registered as a potential focus
   * holder, and the default updater is added.
   */
  ...
  /*
   * Convenience method to create a StringPreference. The widget is registered as a potential focus
   * holder, and the default updater is added.
   * 
   * @since 3.6
   */
   ...
   * unit, or a SnippetPreview which formats several independent code snippets and displays them in
   * the same window.
   * 
   * @param parent Parent composite
   * @return Created preview
   */
  protected abstract DartPreview doCreateJavaPreview(Composite parent);
   ...
   * Subclasses can override this method in order to customize the right-hand side of the dialog.
   * 
   * @param composite Composite to create in
   * @param numColumns Number of columns to use
  ...
  protected Composite doCreatePreviewPane(Composite composite, int numColumns) {
    ...
    final DartPreview preview = doCreateJavaPreview(composite);
  ...
  /**
   * This method is called after all controls have been allocated, including the preview. It can be
   * used to set the preview text and to create listeners.
   */
  ...
  /**
   * Set the status field on the dialog. This can be used by tab pages to report inconsistent input.
   * The OK button is disabled if the kind is IStatus.ERROR.
   * 

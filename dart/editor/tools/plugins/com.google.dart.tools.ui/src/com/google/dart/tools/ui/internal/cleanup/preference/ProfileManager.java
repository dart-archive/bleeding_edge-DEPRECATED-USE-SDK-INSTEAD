...
public abstract class ProfileManager extends Observable {
  ...
  /**
   * Represents a built-in profile. The state of a built-in profile cannot be changed after
   * instantiation.
   */
  public static final class BuiltInProfile extends Profile {
    ...
    public BuiltInProfile(String ID, String name, Map<String, String> settings, int order,
        int currentVersion, String profileKind) {
      fName = name;
      fID = ID;
      fSettings = settings;
      fOrder = order;
  ...
  public final class SharedProfile extends CustomProfile {
    ...
    public SharedProfile(String oldName, Map<String, String> options, int version,
        String profileKind) {
  ...
  /**
   * A prefix which is prepended to every ID of a user-defined profile, in order to differentiate it
  ...
  public ProfileManager(List<Profile> profiles, IScopeContext context,
      PreferencesAccess preferencesAccess, IProfileVersioner profileVersioner, KeySet[] keySets,
      String profileKey, String profileVersionKey) {
    ...
    if (context.getName() == ProjectScope.SCOPE && hasProjectSpecificSettings(context)) {
      ...
      if (map != null) {
        ...
        String projProfileId = context.getNode(DartUI.ID_PLUGIN).get(fProfileKey, null);
  ...
  public void clearAllSettings(IScopeContext context) {
    ...
    final IEclipsePreferences uiPrefs = context.getNode(DartUI.ID_PLUGIN);
    uiPrefs.remove(fProfileKey);
  }

  /**
   * Activate the selected profile, update all necessary options in preferences and save profiles to
   * disk.
   */
  ...
  /**
   * Delete the currently selected profile from this profile manager. The next profile in the list
   * is selected.
   * 
  ...
  /**
   * Get an immutable list as view on all profiles, sorted alphabetically. Unless the set of
   * profiles has been modified between the two calls, the sequence is guaranteed to correspond to
   * the one returned by <code>getSortedNames</code>.
   * 
  ...
  protected String getSelectedProfileId(IScopeContext instanceScope) {
    String profileId = instanceScope.getNode(DartUI.ID_PLUGIN).get(fProfileKey, null);
    if (profileId == null) {
      // request from bug 129427
      profileId = PreferencesAccess.DEFAULT_SCOPE.getNode(DartUI.ID_PLUGIN).get(fProfileKey, null);
    }
    return profileId;
  }
  ...
  protected void updateProfilesWithName(String oldName, Profile newProfile, boolean applySettings) {
    ...
    for (int i = 0; i < projects.length; i++) {
      ...
      IEclipsePreferences node = projectScope.getNode(DartUI.ID_PLUGIN);
    ...
    final IEclipsePreferences uiPrefs = instanceScope.getNode(DartUI.ID_PLUGIN);
   ...
   */
  private Map<String, String> readFromPreferenceStore(IScopeContext context,

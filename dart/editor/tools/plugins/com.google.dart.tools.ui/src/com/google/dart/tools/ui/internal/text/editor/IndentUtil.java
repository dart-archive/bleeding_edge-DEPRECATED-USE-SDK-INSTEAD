...
public final class IndentUtil {
  ...
  /**
   * The result of an indentation operation. The result may be passed to subsequent calls to
   * {@link IndentUtil#indentLines(IDocument, ILineRange, DartProject, IndentResult) indentLines} to
  ...
  public static final class IndentResult {
    ...
    /**
     * Returns <code>true</code> if the indentation operation changed the document,
   ...
   *          comment line properties, or <code>null</code> . Note that the passed result may be
   *          changed by the call.
   * @return an indent result that may be queried for changes and can be reused in subsequent
   *         indentation operations
   * @throws BadLocationException if <code>lines</code> is not a valid line range on
   *           <code>document</code>
   */
  public static IndentResult indentLines(IDocument document, ILineRange lines, DartProject project,
      IndentResult result) throws BadLocationException {
   ...
   *           <code>document</code>
   */
  public static IndentResult shiftLines(IDocument document, ILineRange lines, DartProject project,
      IndentResult result) throws BadLocationException {
    int numberOfLines = lines.getNumberOfLines();
   ...
   */
  private static void addIndent(IDocument document, int line, CharSequence indent,
      boolean[] commentlines, int relative) throws BadLocationException {
  ...
  private static String computeJavadocIndent(IDocument document, int line,
    ...
    if (previousLineNonWS == DartHeuristicScanner.NOT_FOUND
      ...
      // align with the comment start if the previous line is not an asterix
      // line
   ...
   * @throws BadLocationException on concurrent document modification
   */
  private static void cutIndent(IDocument document, int line, int toDelete, int tabSize,
      boolean[] commentLines, int relative) throws BadLocationException {
    IRegion region = document.getLineInformation(line);
   ...
   */
  private static boolean indentLine(IDocument document, int line, DartIndenter indenter,
      DartHeuristicScanner scanner, boolean[] commentLines, int lineIndex, int tabSize)
      throws BadLocationException {
    ...
    // memorize the fact that a line is a single line comment (but not at column
    // 0) and should be treated like code
   ...
   */
  private static int subtractIndent(CharSequence correct, CharSequence current,
      StringBuffer difference, int tabSize) {

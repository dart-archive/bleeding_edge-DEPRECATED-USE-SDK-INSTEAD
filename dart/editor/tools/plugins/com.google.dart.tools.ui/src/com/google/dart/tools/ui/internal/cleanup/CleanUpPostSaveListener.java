...
import com.google.dart.tools.ui.internal.cleanup.CleanUpRefactoring.CleanUpChange;
...
import com.google.dart.tools.ui.internal.cleanup.preference.SaveParticipantPreferencePage;
...
public class CleanUpPostSaveListener implements IPostSaveListener {
  ...
  private static class CleanUpSaveUndo extends TextFileChange {
    ...
    public CleanUpSaveUndo(String name, IFile file, UndoEdit[] undos, long documentStamp,
        long fileStamp) {
      super(name, file);
    ...
    private void performEdit(IDocument document, long oldFileValue,
        LinkedList<UndoEdit> editCollector, long[] oldDocValue, boolean[] setContentStampSuccess)
  ...
  private static final class SlowCleanUpWarningDialog extends OptionalMessageDialog {
    ...
    protected SlowCleanUpWarningDialog(Shell parent, String title, String cleanUpNames) {
      super(ID, parent, title, null, null, MessageDialog.WARNING, new String[] {
     ...
     * 
     * @see
     * org.eclipse.jface.dialogs.IconAndMessageDialog#createMessageArea(org.eclipse.swt.widgets.
     * Composite)
     */
    @Override
...
//  public static final String POSTSAVELISTENER_ID = "org.eclipse.jdt.ui.postsavelistener.cleanup"; //$NON-NLS-1$
  ...
  private static ICleanUp[] getCleanUps(IProject project) throws CoreException {
    ICleanUp[] cleanUps;
    Map<String, String> settings = CleanUpPreferenceUtil.loadSaveParticipantOptions(new ProjectScope(
        project));
//    Map<String, String> settings = loadSaveParticipantOptions(new ProjectScope(project));
    if (settings == null) {
      IEclipsePreferences contextNode = PreferencesAccess.INSTANCE_SCOPE.getNode(DartUI.ID_PLUGIN);
      ...
      if (id == null) {
        id = PreferencesAccess.DEFAULT_SCOPE.getNode(DartUI.ID_PLUGIN).get(
            CleanUpConstants.CLEANUP_ON_SAVE_PROFILE,
...
//          settings.get(CleanUpConstants.CLEANUP_ON_SAVE_ADDITIONAL_OPTIONS));
      ...
      ids.add("com.google.dart.tools.ui.internal.cleanUps"); //$NON-NLS-1$
      ids.add("com.google.dart.tools.ui.cleanUps"); //$NON-NLS-1$
//      ids.add("org.eclipse.jdt.ui.cleanup.imports"); //$NON-NLS-1$
  ...
  private static ICleanUp[] getCleanUps(Map<String, String> settings, Set<String> ids) {
    ICleanUp[] result = DartToolsPlugin.getDefault().getCleanUpRegistry().createCleanUps(ids);
   ...
   * {@inheritDoc}
   */
  public boolean needsChangedRegions(CompilationUnit unit) throws CoreException {
    ICleanUp[] cleanUps = getCleanUps(unit.getDartProject().getProject());
    return requiresChangedRegions(cleanUps);
  }

  @Override
  public void saved(CompilationUnit unit, IProgressMonitor monitor) throws CoreException {
    saved(unit, null, monitor);
  }
   ...
   */
  @Override
  public void saved(CompilationUnit unit, IRegion[] changedRegions, IProgressMonitor monitor)
...
//
//    Map<String, String> compilerOptions = RefactoringASTParser.getCompilerOptions(unit.getDartProject());
...
//  }
  ...
  private IRegion[] performWithChangedRegionUpdate(PerformChangeOperation performChangeOperation,
      IRegion[] changedRegions, CompilationUnit unit, IProgressMonitor monitor)
      throws CoreException {
    final ITextFileBufferManager manager = FileBuffers.getTextFileBufferManager();
...
//  }
  ...
  private CoreException wrapBadPositionCategoryException(BadPositionCategoryException e) {
    ...
    return new CoreException(new Status(IStatus.ERROR, DartUI.ID_PLUGIN, 0, message, e));
  }

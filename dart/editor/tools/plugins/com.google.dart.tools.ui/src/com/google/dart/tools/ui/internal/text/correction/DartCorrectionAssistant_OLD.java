...
public class DartCorrectionAssistant_OLD extends QuickAssistAssistant {
    ...
    if (isInside(invocationLocation, posBegin, posEnd)) { // covers invocation location?
      ...
      if (newClosestPosition != -1) {
        if (newClosestPosition != bestOffset) { // new best
          if (DartCorrectionProcessor_OLD.hasCorrections(annot)) { // only jump to it if there are proposals
  ...
  private QuickAssistLightBulbUpdater_OLD fLightBulbUpdater;
  ...
  public void install(ISourceViewer sourceViewer) {
    ...
    fLightBulbUpdater = new QuickAssistLightBulbUpdater_OLD(fEditor, sourceViewer);

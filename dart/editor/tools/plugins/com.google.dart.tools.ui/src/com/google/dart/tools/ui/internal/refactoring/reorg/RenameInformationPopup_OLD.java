 ...
 */
public class RenameInformationPopup_OLD implements IWidgetTokenKeeper, IWidgetTokenKeeperExtension {
    ...
    public void start() {
      ...
      fPopup.addDisposeListener(new DisposeListener() {
        ...
        public void widgetDisposed(DisposeEvent e) {
          fEditor.getSite().getWorkbenchWindow().getPartService().removePartListener(
   ...
   * https://bugs.eclipse.org/bugs/show_bug.cgi?id=219326 : Shell with custom region and SWT.NO_TRIM
   * still has border
   */
  ...
  private static String getOpenDialogBinding() {
    ...
    String binding = bindingService.getBestActiveBindingFormattedFor(DartEditorActionDefinitionIds.RENAME_ELEMENT);
  ...
  private final RenameLinkedMode_OLD fRenameLinkedMode;
  ...
  public RenameInformationPopup_OLD(CompilationUnitEditor editor, RenameLinkedMode_OLD renameLinkedMode) {
    fEditor = editor;
...
//		fPopup.moveBelow(workbenchShell.getShells()[0]);
  ...
  private ToolBar addViewMenu(final Composite parent) {
    ...
    fMenuImage = DartPluginImages.DESC_ELCL_VIEW_MENU.createImage();
  ...
  private IDialogSettings getDialogSettings() {
    return DartToolsPlugin.getDefault().getDialogSettingsSection(DIALOG_SETTINGS_SECTION);
  }
  ...
  private MenuManager getMenuManager() {
    ...
    fMenuManager.addMenuListener(new IMenuListener2() {
      ...
      public void menuAboutToShow(IMenuManager manager) {
        ...
        IAction openDialogAction = new Action(ReorgMessages.RenameInformationPopup_OpenDialog
            + '\t' + fOpenDialogBinding) {
          @Override
          public void run() {
            activateEditor();
...
//        manager.add(prefsAction);
  ...
  private int[] getPolygon(boolean border) {
    ...
    switch (fSnapPosition) {
      case SNAP_POSITION_OVER_LEFT_FIELD:
        poly = new int[] {
            0, 0, e.x - b, 0, e.x - b, e.y - b, ha1, e.y - b, ha2, e.y + HAH - b, ha3, e.y - b, 0,
            e.y - b, 0, 0};
        break;

      case SNAP_POSITION_UNDER_LEFT_FIELD:
        poly = new int[] {
            0, HAH, ha3 + b, HAH, ha2, b, ha1 - b, HAH, e.x - b, HAH, e.x - b, e.y + HAH - b, 0,
            e.y + HAH - b, 0, HAH};
        break;

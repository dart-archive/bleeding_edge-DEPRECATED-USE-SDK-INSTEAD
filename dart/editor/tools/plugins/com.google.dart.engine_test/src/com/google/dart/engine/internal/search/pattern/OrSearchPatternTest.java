...
public class OrSearchPatternTest extends EngineTestCase {
  ...
  private final SearchPattern pattern = new OrSearchPattern(patternA, patternB);
  ...
  public void test_NameExact() throws Exception {
    ...
    when(patternB.matches(element)).thenReturn(MatchQuality.EXACT);
    // validate
    assertSame(MatchQuality.NAME, pattern.matches(element));
  }

  public void test_NullNull() throws Exception {
    when(patternA.matches(element)).thenReturn(null);

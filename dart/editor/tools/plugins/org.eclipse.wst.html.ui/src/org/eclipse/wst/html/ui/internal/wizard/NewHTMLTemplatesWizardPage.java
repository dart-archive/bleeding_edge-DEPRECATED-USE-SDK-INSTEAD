...
public class NewHTMLTemplatesWizardPage extends WizardPage {
  ...
  public void createControl(Composite ancestor) {
    ...
    PlatformUI.getWorkbench().getHelpSystem().setHelp(parent,
        IHelpContextIds.HTML_NEWWIZARD_TEMPLATE_HELPID);
  ...
  String getTemplateString() {
    ...
    Template template = getSelectedTemplate();
    if (template != null) {
      TemplateContextType contextType = HTMLUIPlugin.getDefault().getTemplateContextRegistry().getContextType(
          TemplateContextTypeIdsHTML.NEW);
  ...
  private void loadLastSavedPreferences() {
    ...
    String templateName = HTMLUIPlugin.getDefault().getPreferenceStore().getString(
        HTMLUIPreferenceNames.NEW_FILE_TEMPLATE_NAME);
    if (templateName == null || templateName.length() == 0) {
      templateName = HTMLUIPlugin.getDefault().getPreferenceStore().getString(
          HTMLUIPreferenceNames.NEW_FILE_TEMPLATE_ID);
      if (templateName != null && templateName.length() > 0) {
  ...
  void saveLastSavedPreferences() {
    ...
    HTMLUIPlugin.getDefault().getPreferenceStore().setValue(
        HTMLUIPreferenceNames.NEW_FILE_TEMPLATE_NAME, templateName);

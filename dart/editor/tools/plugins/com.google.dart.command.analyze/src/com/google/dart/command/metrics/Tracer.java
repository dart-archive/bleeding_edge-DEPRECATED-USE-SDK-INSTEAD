 ...
 * the License.
 */
package com.google.dart.command.metrics;
...
/**
 * Logs performance metrics for internal development purposes. The output is formatted so it can be
 * opened directly in the SpeedTracer Chrome extension. This class formats events using
 * SpeedTracer's custom event feature. The html file output can be viewed by using Chrome to open
 * the file on a Chrome browser that has the SpeedTracer extension installed.
 * <p>
 * Enable logging by setting the system property {@code dart.speedtracerlog} to the output file
 * path.
 * </p>
...
public final class Tracer {
  ...
  static enum Format {
    /**
     * Standard SpeedTracer log that includes JSON wrapped in HTML that will launch a SpeedTracer
     * monitor session.
     */
    HTML,
  ...
  private class GcEvent extends TraceEvent {
     ...
     * @param collectionCount the total number of collections for this garbage collector type
     * @param durationNanos the total elapsed time spent in garbage collection during the span of
     *          {@code refEvent}
     */
    ...
    /**
     * Returns elapsed duration since that is the only duration we can measure for garbage
     * collection events.
     */
    @Override
  ...
  /**
   * Time keeper which uses process cpu time. This can be greater than wall time, since it is
   * cumulative over the multiple threads of a process.
   */
  private class ProcessNormalizedTimeKeeper {
    ...
    public ProcessNormalizedTimeKeeper() {
      try {
        ...
        /*
         * Find this method by reflection, since it's part of the Sun implementation for
         * OperatingSystemMXBean, and we can't always assume that
         * com.sun.management.OperatingSystemMXBean will be available.
         */
  ...
  /**
   * Time keeper which uses per thread cpu time. It is assumed that individual events logged will be
   * single threaded, and that top-level events will call {@link #resetTimeBase()} prior to logging
   * time. The resettable time base is needed since each individual thread starts its timing at 0,
   * regardless of when the thread is created. So we reset the time base at the beginning of an
   * event, so that we can generate a chronologically representative output, although the relation
   * to wall time is actually compressed within a logged event (thread cpu time does not include
   * wait time, etc.).
   */
  private class ThreadNormalizedTimeKeeper {
    ...
    public void resetTimeBase() {
      /*
       * Since all threads start individually at time 0L, we use this to offset each event's time so
       * we can generate chronological output.
       */
  ...
  public class TraceEvent {
    ...
    /**
     * Extends the durations of the current event by the durations of the specified event.
     */
    ...
    /**
     * Returns the event duration, in nanoseconds, for the log file. Depending on system properties,
     * this will measured in elapsed time, process CPU time, or thread CPU time.
     */
    public long getDurationNanos() {
    ...
    /**
     * Returns the event start time, normalized in nanoseconds, for the log file. Depending on
     * system properties, this will be normalized based on elapsed time, process CPU time, or thread
     * CPU time.
     */
    public long getStartTimeNanos() {
    ...
    /**
     * Marks the start time for this event. Three different time measurements are used:
     * <ol>
    ...
    /**
     * Sets the start time of this event to start immediately after the specified event ends.
     */
    void setStartsAfter(TraceEvent refEvent) {
      ...
      processCpuStartTimeNanos = refEvent.processCpuStartTimeNanos
    ...
    /**
     * Records the duration of this event based on the current time and the event's recorded start
     * time.
     */
    void updateDuration() {
  ...
  private static final boolean logProcessCpuTime = getBooleanProperty("dart.speedtracer.logProcessCpuTime");
  ...
  private static final boolean logThreadCpuTime = getBooleanProperty("dart.speedtracer.logThreadCpuTime");
  ...
  private static final boolean logOverheadTime = getBooleanProperty("dart.speedtracer.logOverheadTime");
  ...
  /**
   * Annotate the current event on the top of the stack with more information. The method expects
   * key, value pairs, so there must be an even number of parameters.
   * 
  ...
  /**
   * Returns true if the trace output file is configured. This is intended to be the quickest
   * possible check, statically determined.
   */
  ...
  /**
   * For accessing the logger as a singleton, you can retrieve the global instance. It is prudent,
   * but not necessary to first initialize the singleton with a call to {@link #init()} to set the
   * base time.
   * 
   ...
   * the default logging is turned on with the {@code  -Ddart.speedtracerlog} VM property. This
   * method is only intended to be called once.
   */
  ...
  /**
   * Adds a LOG_MESSAGE SpeedTracer event to the log. This represents a single point in time and has
   * a special representation in the SpeedTracer UI.
   */
   ...
   * {@code start}. You may nest timing calls.
   * 
   * @param type the type of event
   * @param data a set of key-value pairs (each key is followed by its value) that contain
  ...
  /**
   * Notifies the background thread to finish processing all data in the queue. Blocks the current
   * thread until the data is flushed in the Log Writer thread.
   */
  void flush() {
    if (!fileLoggingEnabled) {
      return;
  ...
  TraceEvent startImpl(EventType type, String... data) {
    ...
    if (threadPendingEvents.size() == 0) {
      long baseTime = logProcessCpuTime ? processCpuTimeKeeper.zeroTimeMillis() : (logThreadCpuTime

...
/**
 * Provides methods for checking Dart {@link Element} naming conventions.
 */
...
//    }
    ...
    public String initialChar(String name) {
      return MessageFormat.format(initialChar, name);
    }

    public String internalChar(String name) {
      return MessageFormat.format(internalChar, name);
    }
...
//    }
    ...
    private MessageHolder initForVariable() {
      dollar = "By convention, variable names usually don't contain the $ character";
      empty = "Variable name must not be empty";
      initialCase = "By convention, variable names usually start with a lowercase letter";
      initialChar = "The variable name ''{0}'' is not a valid identifier";
      internalChar = "The variable name ''{0}'' is not a valid identifier";
      leadingOrTrailingBlanks = "A variable name must not start or end with a blank";
      nullName = "Variable name must not be null";
      underscore = "By convention, variable names usually don't contain the underscore character";
      return this;
    }
  }
...
//  }
  ...
  private static RefactoringStatus validateIdentifier(String identifier, MessageHolder messageHolder) {
    int length = identifier.length();
    if (length == 0) {
      return RefactoringStatus.createErrorStatus(messageHolder.empty);
    }
    char currentChar = identifier.charAt(0);
    if (!Character.isLetter(currentChar) && currentChar != '_') {
      return RefactoringStatus.createErrorStatus(messageHolder.initialChar(identifier));
    }
    for (int i = 1; i < length; i++) {
      currentChar = identifier.charAt(i);
      if (!Character.isLetterOrDigit(currentChar) && currentChar != '_') {
        return RefactoringStatus.createErrorStatus(messageHolder.internalChar(identifier));
      }
    }
   ...
   */
  private static RefactoringStatus validateLowerCamelCase(String identifier,
      MessageHolder messageHolder) {
    // null
    if (identifier == null) {
      return RefactoringStatus.createErrorStatus(messageHolder.nullName);
    }
    // has leading/trailing spaces
    String trimmed = identifier.trim();
    if (!identifier.equals(trimmed)) {
      return RefactoringStatus.createErrorStatus(messageHolder.leadingOrTrailingBlanks);
    }
    // is not identifier
    RefactoringStatus status = validateIdentifier(identifier, messageHolder);
    if (!status.isOK()) {
      return status;
    }
    // is private, OK
    ...
    if (!Character.isLowerCase(identifier.charAt(0))) {
      return RefactoringStatus.createWarningStatus(messageHolder.initialCase);
    }
    // OK
    return new RefactoringStatus();
  }

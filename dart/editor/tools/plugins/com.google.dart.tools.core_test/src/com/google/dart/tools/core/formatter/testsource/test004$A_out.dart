 ...
 * Class CFG
 *
 * A simple class simulating the concept of a control flow graph.
 *
 * CFG maintains a list of nodes, plus a start node. That's it.
 */
class CFG {
  ...
  CFG() {
    ...
    edgeList = new QueueImplementation<BasicBlockEdge>(); // error if
// < is (
  }
  ...
  void dump() {
    basicBlockMap.forEach(
      function f(int n, BasicBlock bb) {
        bb.dump();
      });
  }
 ...
 *
 * These data structures are stubbed out to make the code below easier to
 * review.
 *
 ...
 *
 * This program constructs a fairly large control flow graph and performs loop
 * recognition. This is the Dart version.
 *
...
class Havlak {
  ...
  /**
   * Create 4 basic blocks, corresponding to and if/then/else clause with a CFG
...
class HavlakLoopFinder {
   ...
   * IsAncestor
   *
   * As described in the paper, determine whether a node 'w' is a "true"
   * ancestor for node 'v'.
   *
   * Dominance can be tested quickly using a pre-order trick for depth-first
   * spanning trees. This is why DFS is the first thing we run below.
   */
   ...
   *
   * DESCRIPTION: Simple depth first traversal along out edges with node
   * numbering.
   */
  int doDFS(BasicBlock currentNode, Array<UnionFindNode> nodes, Map<BasicBlock, int> number,
      Array<int> last, int current) {
    nodes[current].initNode(currentNode, current);
    number[currentNode] = current;
    int lastid = current;
    currentNode.getOutEdges().forEach(
      function f(BasicBlock target) {
   ...
   * findLoops
   *
   * Find loops and build loop forest using Havlak's algorithm, which is derived
   * from Tarjan. Variable names and step numbering has been chosen to be
   * identical to the nomenclature in Havlak's paper (which, in turn, is similar
  ...
  void findLoops() {
    ...
    cfg.getBasicBlocks().forEach(
      function f(int n, BasicBlock bbIter) {
    ...
    for (int w = 0; w < size; w++) {
      ...
      if (nodeW.getNumPred() > 0) {
        nodeW.getInEdges().forEach(
          function f(BasicBlock nodeV) {
    ...
    for (int w = size - 1; w >= 0; w--) {
      ...
      nodePool.forEach(
        Function f(UnionFindNode niter) {
      ...
      while (!workList.isEmpty) {
        ...
        nonBackPreds[x.getDfsNumber()].forEach(
          function f(int iter) {
 ...
 * Class UnionFindNode
 *
 * The algorithm uses the Union/Find algorithm to collapse complete loops into a
 * single node. These nodes and the corresponding functionality are implemented
 * with this class.
 */
class UnionFindNode {
   ...
   *
   * Implemented with Path Compression (inner loops are only visited and
   * collapsed once, however, deep nests would still result in significant
   * traversals).
   */
 ...
 *
 * Two values are maintained for this loop graph, depth, and nesting level. For
 * example:
 *
 * loop nesting level depth---------------------------------------- loop-0 2 0
 * loop-1 1 1 loop-3 1 1 loop-2 0 2
 */
class LSG {
  SimpleLoop root;
  ...
  LSG() :
      loopCounter = 0,
      loops = new GrowableArray<SimpleLoop>(),
      root = new SimpleLoop() {
 ...
 * Class SimpleLoop
 *
 * Basic representation of loops, a loop has an entry point, one or more exit
 * edges, a set of basic blocks, and potentially an outer loop - a "parent"
 * loop.
 *
 * Furthermore, it can have any set of properties, e.g., it can be an
 * irreducible loop, have control flow, be a candidate for transformations, and
 * what not.
 */
class SimpleLoop {

...
public class NewSearchEngineTest extends TestCase {
  ...
  public void test_SearchEngine_searchImplementors() throws Exception {
    ...
    SearchEngine engine = createSearchEngine();
    List<SearchMatch> matches = engine.searchImplementors(type,
        SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
    assertEquals(2, matches.size());
  }
  ...
  public void test_SearchEngine_searchMethodDeclarations() throws Exception {
    ...
    List<SearchMatch> matches = engine.searchMethodDeclarations(
        ...
        SearchPatternFactory.createPrefixPattern("ad", true), null, new NullProgressMonitor());
  ...
  public void test_SearchEngine_searchReferences_field() throws Exception {
    ...
    SearchEngine engine = createSearchEngine();
    List<SearchMatch> matches = engine.searchReferences(field,
        SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
  ...
  public void test_SearchEngine_searchReferences_field_local() throws Exception {
    ...
    try {
      String source = buildSource(
          "// filler filler filler filler filler filler filler filler filler filler", "class A {",
          "  var test;", "}", "class B extends A {", "  foo() {", "    test = 1;",
          "    this.test = 2;", "  }", "}", "bar() {", "  A a = new A();", "  a.test = 3;", "}", "");
      ...
      // assert "qualified"
      Map<Integer, Boolean> expected = ImmutableMap.of(source.indexOf("test = 1"), false,
          source.indexOf("test = 2"), true, source.indexOf("test = 3"), true);
  ...
  public void test_SearchEngine_searchReferences_function() throws Exception {
    ...
    try {
      CompilationUnit unit = testProject.setUnitContent(
          "Test.dart",
          buildSource("// filler filler filler filler filler filler filler filler filler filler",
              "test() {}", "foo() {", "  test();", "}", ""));
      indexUnits(unit);
      DartFunction function = (DartFunction) unit.getChildren()[0];
      SearchEngine engine = createSearchEngine();
      List<SearchMatch> matches = engine.searchReferences(function,
          SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
  ...
  public void test_SearchEngine_searchReferences_globalVarible_argumentQualifiedInvocation()
    ...
    try {
      String source = buildSource(
          "// filler filler filler filler filler filler filler filler filler filler", "var test;",
          "f() {", "  var server;", "  server.listen(test);", "}", "");
      CompilationUnit unit = testProject.setUnitContent("Test.dart", source);
      indexUnits(unit);
      // find references
      DartVariableDeclaration variable = (DartVariableDeclaration) unit.getChildren()[0];
      SearchEngine engine = createSearchEngine();
      List<SearchMatch> matches = engine.searchReferences(variable,
          SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
  ...
  public void test_SearchEngine_searchReferences_import_noPrefix() throws Exception {
    ...
    try {
      testProject.setUnitContent(
          "Lib.dart",
          buildSource("// filler filler filler filler filler filler filler filler filler filler",
              "#library('libA');", "class LibType {", "  static var staticField;", "}",
              "var libVar;", "libFunction() {}", "")).getResource();
      IResource resourceTest = testProject.setUnitContent(
          "TestC.dart",
          buildSource("// filler filler filler filler filler filler filler filler filler filler",
              "#library('Test');", "#import('Lib.dart');", "f() {", "  LibType v;",
              "  LibType.staticField = 1;", "  libVar = 0;", "  libFunction();", "}", "")).getResource();
  ...
  public void test_SearchEngine_searchReferences_import_noPrefix_twoLibraries() throws Exception {
    ...
    try {
      ...
      IResource resourceTest = testProject.setUnitContent(
          "TestC.dart",
          buildSource("// filler filler filler filler filler filler filler filler filler filler",
              "#library('Test');", "#import('LibA.dart');", "#import('LibB.dart');", "f() {",
              "  A a;", "  B b;", "}", "")).getResource();
      DartLibrary libraryTest = testProject.getDartProject().getDartLibrary(resourceTest);
      // index unit
      CompilationUnit unit = libraryTest.getDefiningCompilationUnit();
  ...
  public void test_SearchEngine_searchReferences_import_reusePrefix() throws Exception {
    ...
    try {
      ...
      IResource resourceTest = testProject.setUnitContent(
          "TestC.dart",
          buildSource("// filler filler filler filler filler filler filler filler filler filler",
              "#library('Test');", "#import('LibA.dart', prefix: 'aaa');",
              "#import('LibB.dart', prefix: 'aaa');", "f() {", "  aaa.A a;", "  aaa.B b;", "}", "")).getResource();
      DartLibrary libraryTest = testProject.getDartProject().getDartLibrary(resourceTest);
      // index unit
      CompilationUnit unit = libraryTest.getDefiningCompilationUnit();
      indexUnits(unit);
  ...
  public void test_SearchEngine_searchReferences_import_uniquePrefixes() throws Exception {
    ...
    try {
      ...
      IResource resourceTest = testProject.setUnitContent(
          ...
          buildSource("// filler filler filler filler filler filler filler filler filler filler",
              "#library('Test');", "#import('LibA.dart', prefix: 'aaa');",
              "#import('LibB.dart', prefix: 'bbb');", "f() {", "  aaa.A a;", "  bbb.B b;", "}", "")).getResource();
      DartLibrary libraryTest = testProject.getDartProject().getDartLibrary(resourceTest);
      // index unit
      CompilationUnit unit = libraryTest.getDefiningCompilationUnit();
      indexUnits(unit);
  ...
  public void test_SearchEngine_searchReferences_method() throws Exception {
    ...
    try {
      String source = buildSource(
          "// filler filler filler filler filler filler filler filler filler filler", "class A {",
          "  test(var p) {}", "}", "class B extends A {", "  foo() {", "    test(1);",
          "    this.test(2);", "  }", "}", "bar() {", "  A a = new A();", "  a.test(3);", "}", "");
      CompilationUnit unit = testProject.setUnitContent("Test.dart", source);
      indexUnits(unit);
      // find references
      ...
      SearchEngine engine = createSearchEngine();
      List<SearchMatch> matches = engine.searchReferences(method,
          SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
      assertThat(matches).hasSize(3);
      // assert "qualified"
      Map<Integer, Boolean> expected = ImmutableMap.of(source.indexOf("test(1"), false,
      ...
      for (SearchMatch match : matches) {
        int matchOffset = match.getSourceRange().getOffset();
        assertEquals(match.toString(), expected.get(matchOffset).booleanValue(),
            match.isQualified());
      }
    } finally {
      testProject.dispose();
  ...
  public void test_SearchEngine_searchReferences_type() throws Exception {
    Type type = moneyLibrary.getCompilationUnit("simple_money.dart").getType("SimpleMoney");
    SearchEngine engine = createSearchEngine();
    List<SearchMatch> matches = engine.searchReferences(type,
        SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
  ...
  public void test_SearchEngine_searchReferences_type_fromConstructor_factoryImpl()
    ...
    try {
      String source = buildSource(
          ...
          "interface I factory F { // marker-1", "  I();", "  I.named();", "}",
          "class F implements I { // marker-2", "  factory F() {}", "  factory F.named() {}", "}",
          "");
      CompilationUnit unit = testProject.setUnitContent("Test.dart", source);
      indexUnits(unit);
      // references to "I"
  ...
  public void test_SearchEngine_searchReferences_type_fromConstructor_factoryNoImpl()
    ...
    try {
      String source = buildSource(
          "// filler filler filler filler filler filler filler filler filler filler",
          "interface I factory F { // marker-1", "  I();", "  I.named();", "}", "class F {",
          "  factory I() {}", "  factory I.named() {}", "}", "");
      CompilationUnit unit = testProject.setUnitContent("Test.dart", source);
      indexUnits(unit);
      // references to "I"
      {
        Type typeI = (Type) unit.getChildren()[0];
        assertTypeReferences(typeI, 1,
  ...
  public void test_SearchEngine_searchReferences_variable() throws Exception {
    ...
    try {
      CompilationUnit unit = testProject.setUnitContent(
          "Test.dart",
          buildSource("int globalCount;", "", "void main() {", "  globalCount = 0;",
              "  print(globalCount);", "}", ""));
      indexUnits(unit);
      DartVariableDeclaration variable = (DartVariableDeclaration) unit.getChildren()[0];
      SearchEngine engine = createSearchEngine();
  ...
  public void test_SearchEngine_searchTypeDeclarations_multipleLibraries() throws Exception {
    TestProject testProject = new TestProject("Test");
    try {
      CompilationUnit a = testProject.setUnitContent("a.dart",
          buildSource("#library('a');", "class Class1 {}"));
      CompilationUnit b = testProject.setUnitContent("b.dart",
          buildSource("#library('b');", "class Class2 {}", "class Class3 {}"));
      CompilationUnit c = testProject.setUnitContent(
          "c.dart",
          buildSource("#library('c');", "#import('a.dart');", "class Class4 {}", "class Class5 {}",
              "class Class6 {}"));
      indexUnits(a, b, c);
      ...
      List<SearchMatch> matches = engine.searchTypeDeclarations(
          ...
          SearchPatternFactory.createPrefixPattern("Cla", true), (SearchFilter) null,
  ...
  public void test_SearchEngine_searchTypeDeclarations_singleLibrary_emptyPattern()
    ...
    try {
      ...
      List<SearchMatch> matches = engine.searchTypeDeclarations(
          SearchScopeFactory.createLibraryScope(c.getLibrary()),
          SearchPatternFactory.createPrefixPattern("", true), (SearchFilter) null,
          new NullProgressMonitor());
      assertEquals(3, matches.size());
    } finally {
      testProject.dispose();
  ...
  public void test_SearchEngine_searchTypeDeclarations_singleLibrary_nullPattern() throws Exception {
    ...
    try {
      ...
      SearchEngine engine = createSearchEngine();
      List<SearchMatch> matches = engine.searchTypeDeclarations(
          SearchScopeFactory.createLibraryScope(c.getLibrary()), null, (SearchFilter) null,
          new NullProgressMonitor());
      assertEquals(3, matches.size());
    } finally {
      testProject.dispose();
  ...
  private void assertTypeReferences(Type type, int length, String[] refMarkers) throws Exception {
    ...
    // find references
    SearchEngine engine = createSearchEngine();
    List<SearchMatch> references = engine.searchReferences(type,
        SearchScopeFactory.createWorkspaceScope(), null, new NullProgressMonitor());
  ...
  private List<SearchMatch> getImportReferences(DartImport dartImport) throws SearchException {
    SearchEngine engine = createSearchEngine();
    return engine.searchReferences(dartImport, SearchScopeFactory.createWorkspaceScope(), null,
        new NullProgressMonitor());
  }
  ...
  private void prepare_searchReferences_import(TestProject testProject) throws Exception {
    testProject.setUnitContent(
        "LibA.dart",
        buildSource("// filler filler filler filler filler filler filler filler filler filler",
            "#library('libA');", "class A {}", "")).getResource();
    testProject.setUnitContent(
        "LibB.dart",
        buildSource("// filler filler filler filler filler filler filler filler filler filler",
            "#library('libB');", "class B {}", "")).getResource();
  }
}

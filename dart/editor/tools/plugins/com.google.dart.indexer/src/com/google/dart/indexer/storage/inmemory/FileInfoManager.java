...
final class FileInfoManager {
  ...
  public FileInfoManager(int i, AbstractStringEncoder optimizedStringEncoder) {
    this.ids = new int[i];
  ...
  public FileInfoManager(int i, ByteArray array2) {
    this.ids = new int[i];
    array = array2;
  }

  public FileInfoManager(int size2, int[] ids2, IntArray locInfos, IntArray poss, ByteArray array2,
      PlainStringEncoder plainStringEncoder) {
    this.array = array2;
    this.ids = ids2;
  ...
  public boolean equals(Object obj) {
    ...
    FileInfoManager other = (FileInfoManager) obj;
    int where = 0;
    boolean result = size == other.size && (array.equals(other.array) && (++where > 0))
        && (Arrays.equals(ids, other.ids) && (++where > 0))
        && (positions.equals(other.positions) && (++where > 0))
        && (locationInfos.equals(other.locationInfos) && (++where > 0));
    if (!result) {
  ...
  void setFileInfo(int p, int value) {
  ...
  private void rehash() {
    FileInfoManager simpleStringPool = new FileInfoManager(this.ids.length * 2, this.array);

...
/**
 * Utility class that supports random and secure random functions. In some systems SecureRandom
 * initialization is very slow, a workaround is implemented here.
 */
public class RandomUtils {
  ...
  /**
   * Get a pseudo random int value between 0 (including and the given value (excluding). The value
  ...
  /**
   * Get a cryptographically secure pseudo random int value between 0 (including and the given value
   * (excluding).
   * 
   * @param lowerThan the value returned will be lower than this value
   * @return the random long value
  ...
  /**
   * Print a message to system output if there was a problem initializing the random number
   * generator.
   * 
   * @param s the message to print
   * @param t the stack trace
   */
  static void warn(String s, Throwable exception) {
    // not a fatal problem, but maybe reduced security
    IndexerPlugin.getLogger().trace(IndexerDebugOptions.MISCELLANEOUS, "RandomUtils warning: " + s);
    if (exception != null) {
      IndexerPlugin.getLogger().logWarning(exception, "RandomUtils warning: " + s);
    }
  }
  ...
  private static byte[] generateAlternativeSeed() {
    try {
      ...
      try {
        ...
        Object localHost = inetAddressClass.getMethod("getLocalHost", new Class[0]).invoke(null,
            new Object[0]);
        String hostName = inetAddressClass.getMethod("getHostName", new Class[0]).invoke(localHost,
            new Object[0]).toString();
        out.writeUTF(hostName);
        Object[] list = (Object[]) inetAddressClass.getMethod("getAllByName",
            new Class[] {String.class}).invoke(null, new Object[] {hostName});

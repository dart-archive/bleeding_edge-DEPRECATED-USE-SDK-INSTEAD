...
public class NewCSSTemplatesWizardPage extends WizardPage {
  ...
  public void createControl(Composite ancestor) {
    ...
    PlatformUI.getWorkbench().getHelpSystem().setHelp(parent,
        IHelpContextIds.CSS_NEWWIZARD_TEMPLATE_HELPID);
  ...
  String getTemplateString() {
    ...
    Template template = getSelectedTemplate();
    if (template != null) {
      TemplateContextType contextType = CSSUIPlugin.getDefault().getTemplateContextRegistry().getContextType(
          TemplateContextTypeIdsCSS.NEW);
  ...
  private void loadLastSavedPreferences() {
    ...
    String templateName = CSSUIPlugin.getDefault().getPreferenceStore().getString(
        CSSUIPreferenceNames.NEW_FILE_TEMPLATE_NAME);
    if (templateName == null || templateName.length() == 0) {
      templateName = CSSUIPlugin.getDefault().getPreferenceStore().getString(
          CSSUIPreferenceNames.NEW_FILE_TEMPLATE_ID);
      if (templateName != null && templateName.length() > 0) {
  ...
  void saveLastSavedPreferences() {
    ...
    CSSUIPlugin.getDefault().getPreferenceStore().setValue(
        CSSUIPreferenceNames.NEW_FILE_TEMPLATE_NAME, templateName);
